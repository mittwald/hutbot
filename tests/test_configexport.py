...
def test_an_error_that_suggests_what_to_paste_names_this_instances_command():
    ...
    assert "/hutbot_dev [config] config export" in error


def test_a_whole_channel_round_trips_through_read_entries_in_order():
    configs = {"nightly": {**DEFAULT_CONFIG, "wait_time": 600},
               "alarms": {**DEFAULT_CONFIG, "cron": "0 9 * * 1-5"}}
    dumped = configexport.dump_payload(configexport.build_multi_payload(configs))
    entries, error = configexport.read_entries(dumped, "/hutbot")
    assert not error
    # The order the channel keeps them in, not sorted, so the import reads like the export.
    assert entries == [("nightly", {"wait_time": 600}), ("alarms", {"cron": "0 9 * * 1-5"})]


def test_one_config_and_a_whole_channel_take_the_same_shape_out_of_read_entries():
    config = {**DEFAULT_CONFIG, "wait_time": 600}
    dumped = configexport.dump_payload(configexport.build_payload("nightly", config))
    assert configexport.read_entries(dumped, "/hutbot") == \
        ([("nightly", {"wait_time": 600})], "")


def test_an_all_configs_export_has_nowhere_to_go_in_a_single_config_import():
    # The App Home's import modal imports into one named rule; half-applying a channel's
    # worth of them is worse than saying so.
    configs = {"nightly": {**DEFAULT_CONFIG}, "alarms": {**DEFAULT_CONFIG}}
    dumped = configexport.dump_payload(configexport.build_multi_payload(configs))
    settings, name, error = configexport.read_payload(dumped, "/hutbot")
    assert settings is None and "holds 2 configurations" in error
    assert "/hutbot config import" in error


@pytest.mark.parametrize("text,expected", [
    ('{"format": "hutbot-config/1", "configs": []}', "non-empty JSON array"),
    ('{"format": "hutbot-config/1", "configs": {}}', "non-empty JSON array"),
    ('{"format": "hutbot-config/1", "configs": [{"name": "x"}]}',
     "needs a `name` and a `settings` object"),
])
def test_an_unreadable_all_configs_export_says_what_is_wrong_with_it(text, expected):
    entries, error = configexport.read_entries(text, "/hutbot")
    assert entries is None and expected in error
