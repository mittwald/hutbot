...
from bot import (
    ...
    set_replies_enabled,
    validate_config_payload, get_channel_members, is_user_in_channel,
    list_user_config_channels, ui_create_config, ui_delete_config,
)
...
# ---------------------------------------------------------------------------
# Web-UI backend helpers: validate_config_payload / membership / ui create+delete
# ---------------------------------------------------------------------------
import bot
...
def _seed_user_caches():
    ...
    bot.team_cache = {"Platform", "Support"}
    bot.user_id_cache = {"someuser": bot.User(id="U1", name="someuser", real_name="X", team="Platform")}
    bot.user_email_cache = {"x@example.com": bot.User(id="U1", name="someuser", real_name="X", team="Platform")}
    bot.id_user_cache = {"U1": bot.User(id="U1", name="someuser", real_name="X", team="Platform")}
...
async def test_list_user_config_channels_filters_by_membership():
    bot._channel_members_cache = {}
    bot.channel_config = {"C1": {"default": {}}, "C2": {"default": {}}}
    app = _ui_app()
    ...
    user = bot.User(id="U1", name="someuser", real_name="X", team="Platform")
...
async def test_ui_create_config_rejects_bad_name_and_duplicate(monkeypatch):
    bot.channel_config = {}
    monkeypatch.setattr(bot, "save_configuration", AsyncMock())
    app = _ui_app()

    # bad name (space not allowed by the name pattern)
    ok, msg = await bot.ui_create_config(app, "C1", "bad name")
    assert ok is False and msg

    # good name succeeds and seeds from DEFAULT_CONFIG
    ok, msg = await bot.ui_create_config(app, "C1", "my-rule")
    assert ok is True
    assert bot.channel_config["C1"]["my-rule"]["reply_message"] == bot.DEFAULT_CONFIG["reply_message"]
    bot.save_configuration.assert_awaited()
...
async def test_ui_delete_config_refuses_default(monkeypatch):
    bot.channel_config = {"C1": {"default": {}, "my-rule": {}}}
    monkeypatch.setattr(bot, "save_configuration", AsyncMock())
    app = _ui_app()

    ok, msg = await bot.ui_delete_config(app, "C1", "default")
    assert ok is False and msg
    assert "default" in bot.channel_config["C1"]

    ok, msg = await bot.ui_delete_config(app, "C1", "my-rule")
    assert ok is True
    assert "my-rule" not in bot.channel_config["C1"]
